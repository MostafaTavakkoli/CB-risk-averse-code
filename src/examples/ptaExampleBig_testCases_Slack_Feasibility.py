...
for k_mpc in range(2):
    ...
    if k_mpc>0:
        ...
        consistency = pd.Constr(prevState.gloClock.name, '==', globalTime)
    ...
    if horizon == 5:
        ######### horizon = 4 #####
        solver = ps.ptaSolver(A,horizon, 'min',[1,0],uk) if k_mpc <= 0 else ps.ptaSolver(A,horizon, 'min',[0,1],uk)
        ###########################
    elif horizon == 6:
        ######### horizon = 6 #####
        if k_mpc <= 3:
    ...
    Us, localClocks, globalClocks, slackVars, execT, z1 = solver.minTimeReach()
    ...
    uk[Us[horizon-1]] = 1
    Ulist.append(uk)
    prevState = transList[Us[horizon-1]].source # this is the state before the current actually

    # update the time
    # globalTime += localClocks[currentState.name+'_local']
    globalTime = globalClocks[prevState.name+'_globl']
