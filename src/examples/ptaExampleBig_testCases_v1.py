...
import src.ptaSolver_wExpSlack as ps
import numpy as np
...
for k_mpc in range(Nmpc):
    ...
    c4 = pd.Constr(tcnc1+4-globalTime, '<=', AGV2_1.gloClock.name)
    c5 = pd.Constr(1, '<=', AGV2_1.locClock.name)
    c6 = pd.Constr(tcnc2, '<=', CNC2.locClock.name)
    c7 = pd.Constr(tcnc1+11-globalTime, '<=', AGV2_2.gloClock.name)
    c8 = pd.Constr(1, '<=', AGV2_2.locClock.name)
    co1l = pd.Constr(tqc,'<=', qCheck.locClock.name)
    co1g = pd.Constr(tqc-globalTime,'<=', qCheck.gloClock.name)
    ...
    # tf = pd.Constr(23-globalTime,'>=', tBuffr.gloClock.name)
    # tBuffr.invs = [tf]
    #################################################
    ...
    ######### horizon = 4 #####
    # solver = ps.ptaSolver(A, 'min',[1,0]) if k_mpc <= 3 else ps.ptaSolver(A, 'min',[0,1])
    ###########################
    ######### horizon = 6 #####
    # if k_mpc < 3:
    #     solver = ps.ptaSolver(A, 'min',[1,0])
    # elif k_mpc == 3:
    #     solver = ps.ptaSolver(A, 'min',[1,1])
    # else:
    #     solver = ps.ptaSolver(A, 'min',[0,1])
    ###########################
    ######### horizon = 8 #####
    solver = ps.ptaSolver(A, 'min',[1,1]) if k_mpc <= 3 else ps.ptaSolver(A, 'min',[0,1])
    ###########################

    Us, localClocks, globalClocks, slackVars, execT = solver.solvePTA()
